...
# Initialize points and types for two particle types
...
points, types = init(points, types)
...
pygame.display.set_caption("Multi-Particle-Type Simulation")
...
while running:
    ...
    for i in range(point_n):
        ...
        # Particles of type 0 and 1 have different size scaling
        if types[i] == 0:
            r = 5 + fields["R_val"][i] * 10  # Size based on repulsion for type 0
            color = (255, 0, 0)  # Red for type 0
        else:
            r = 7 + fields["R_val"][i] * 8  # Size based on repulsion for type 1
            color = (0, 0, 255)  # Blue for type 1
